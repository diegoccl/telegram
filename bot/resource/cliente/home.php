...
function clientes($message){
	...
	if ($cmd == 'start'){
   	...
   	$clientes = json_decode(file_get_contents("./usuarios.json") , true);
		...
		[['text'=>"💳 Comprar",'callback_data'=>"loja"] , ['text'=>"👤 Informações",'callback_data'=>"menu_infos"]]
...
function menu_infos($message){
	...
	$txt .= "📥 *Ultima Atualização:* {$conf[updateDAte]}\n";
...
function comprasaldo($message){
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"Volta",'callback_data'=>"volta_loja"] , ['text'=>'Comprovate','url'=>'https://t.me/prazeramigavel']]
...
bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "Pix numero: 21974088351\nNome : Stephany\nBanco : Pagbank\n\nEnvie seu comprovate para {$confibot[userDono]}","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown' , 'force_reply' => true , "selective" => true));
...
function altercard($message , $query , $type , $position , $level , $band){
	...
	$menu =  ['inline_keyboard' => [
		...
		[["text" => "💰Comprar💰" , "callback_data" => "compraccs_{$level}_{$dadoscc[idcc]}_{$band}"]],
...
function loja($message){
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"🔀 MIX",'callback_data'=>"ccmix"] , ['text'=>"🔎 Pesquisar bin",'callback_data'=>"search"]],
	...
	bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "💳 *Olá $nome, bem-vindo(a) a minha loja virtual! Aqui você pode comprar unidades, mix, saldo e muito mais.*\n\n👉 *Confira as opções abaixo:*","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
...
function menu($message){
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"🛒 Loja virtual 🛒",'callback_data'=>"loja"] , ['text'=>"💎 Seu perfil 💎",'callback_data'=>"menu_infos"]]
...
function ccun($message){
	...
	bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "*💳 Escolha o nivel preferido:*\n","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
